...
function sumLinkedListsForward(list1, list2) {
  ...
  const { head, nextDigitValue } = carryBase10(
    sumAndAppendNodes(list1, list2),
    0
  )
  return nextDigitValue
    ? appendToStart(head, new LinkedList(nextDigitValue))
    : head
}
...
function sumAndAppendNodes(node1, node2) {
  ...
  const { head, nextDigitValue } = carryBase10(
    sumAndAppendNodes(node1.next, node2.next),
    value
  )
