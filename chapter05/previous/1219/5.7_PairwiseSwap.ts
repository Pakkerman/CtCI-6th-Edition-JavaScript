...
export function solution(n: number): number {
  ...
  // 0xa = 1010, repeat 8 times to capture all even bits in a 32-bit number
  // 0x5 = 0101, repeat 8 times to capture all odd bits in a 32-bit number
  console.log(n.toString(2))
  const even = n & 0xaaaaaaaa
  console.log(even.toString(2).padStart(n.toString(2).length, '0'))

  const odd = n & 0x55555555
  // shift even left, odd to the right by 1, OR them together
  return (even >>> 1) | (odd << 1)
}
