...
export function getNext(n: number): number {
  ...
  n |= 0b1 << flipIdx // flip that bit
  n &= ~((1 << flipIdx) - 1) // clear flipIdx to 0th
  n |= (1 << (trailingOnes - 1)) - 1 // adding ones but minus one because we alread flipped one
  return n
}
...
export function getPrevious(n: number): number {
  ...
  console.log('Zeros: \t\t', trailingZeros)
  console.log('Ones: \t\t', trailingOnes)
  const flipIdx = trailingOnes + trailingZeros
  console.log('FlipIdx: \t', flipIdx)

  // flip 8th bit to 0

  console.log('n: \t\t', n.toString(2))
  n &= ~(1 << flipIdx)
  console.log('flipped: \t', n.toString(2))
  n &= ~0 << flipIdx
  console.log('cleared: \t', n.toString(2))
  // we need trailing ones + 1 of 1s to be added back, trailingOnes + flipped bit
  n |= ((1 << (trailingOnes + 1)) - 1) << (trailingZeros - 1)
  console.log('add back: \t', n.toString(2))
  return n
}
