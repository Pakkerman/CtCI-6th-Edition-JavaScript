...
export function clearBit(num: number, i: number): number {
  // 11110001, clear first bit
  const mask = ~(0b1 << i) // 000000001 >> 11111110
  // 11110001 & 11111110, the 1s will not change but the i bit will be set to 0
  return num & mask
}
...
export function clearBitsMSBToIth(num: number, i: number): number {
  // 11111010, i = 2
  const mask = (0b1 << i) - 1 // 00000100 - 1 >> 00000011
  // 11111010 & 00000011 >> 00000010
  return mask & num
}
...
export function clearBitsIthTo0(num: number, i: number): number {
  // 11110011, i = 2
  const mask = ~0b0 << (i + 1) // 11111111 > 11110000
  // ~-1 or ~0b0 both works
